output "nomad_ip" {
  value = "http://${aws_instance.nomad_leader[0].public_ip}:4646/ui"
}
...
http://${aws_instance.nomad_leader[0].public_ip}:4646/ui
Set the Nomad address, run the bootstrap, export the management token, set the token variable, and test connectivity:
export NOMAD_ADDR=http://${aws_instance.nomad_leader[0].public_ip}:4646 && \
